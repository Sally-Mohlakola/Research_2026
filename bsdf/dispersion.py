...
MI_CIE_MIN = float(mi.MI_CIE_MIN)
MI_CIE_MAX = float(mi.MI_CIE_MAX)
...
# ─────────────────────────────────────────────
# Spectral throughput -> RGB, for a dispersive gather
# ─────────────────────────────────────────────
...
def _white_norm():
    """
    Scale that makes an equal-energy spectrum map to RGB (1, 1, 1).

    `mi.spectrum_to_srgb` carries the CIE normalisation and a 1/N average, so
    a unit spectrum comes back at ~1e-3 rather than 1. Dividing by this
    constant keeps the RDM on the same energy scale it has always used --
    gathered albedos summing to ~1, and therefore the studio rig's exposure
    calibration -- while letting hue vary. Without it, switching the gather to
    a dispersive BSDF would silently rescale every stored value by ~1000x.
    """
    global _WHITE_NORM
    if _WHITE_NORM is None:
        n = 1 << 16
        u = mi.Float(np.linspace(0.0, 1.0, n, endpoint=False).astype(np.float32))
        wl = MI_CIE_MIN + mi.sample_shifted(u) * (MI_CIE_MAX - MI_CIE_MIN)
        rgb = mi.spectrum_to_srgb(mi.UnpolarizedSpectrum(1.0), wl)
        lum = 0.2126 * rgb.x + 0.7152 * rgb.y + 0.0722 * rgb.z
        _WHITE_NORM = float(dr.mean(lum)[0])
    return _WHITE_NORM


def spectral_to_rgb(value, wavelengths):
    """
    Convert a spectral throughput to RGB, preserving hue.

    This is the colour-carrying counterpart of `to_rgb3`. That function
    collapses every channel to channel 0 and returns a grey triple, which is
    exact only because the default gather scene uses an achromatic
    `dielectric` -- and is precisely why `rdm_m` comes back with a per-bin
    channel spread of exactly zero and the neural render has no colour of its
    own. Gathering through `DispersiveDielectric` and reducing with this
    function instead is what gives the RDM chromatic variation to learn.
    """
    if not IS_SPECTRAL:
        return mi.Color3f(value.x, value.y, value.z)
    # Deliberately NOT clamped here. A single-wavelength path is usually
    # outside the sRGB gamut -- 450 nm converts to a negative green -- and
    # clamping each sample before it is accumulated stops those negatives ever
    # cancelling, which biases every bin toward oversaturation. The clamp
    # belongs after the per-bin average, in compute_histogram_4d.
    return mi.spectrum_to_srgb(value, wavelengths) / _white_norm()


def sample_gather_wavelengths(n, seed=0):
    """
    Wavelengths for `n` gather rays, uniform over the CIE range.

    Mitsuba's `sample_shifted` turns one uniform variate into N correlated
    wavelengths, which is the same hero-plus-rotations scheme the renderer
    uses, so the gather samples the spectrum the way the render will.
    """
    rng = np.random.default_rng(seed)
    u = mi.Float(rng.random(n).astype(np.float32))
    return MI_CIE_MIN + mi.sample_shifted(u) * (MI_CIE_MAX - MI_CIE_MIN)
