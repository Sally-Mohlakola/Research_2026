...
class NeuralDiamond(mi.BSDF):
    ...
    def __init__(self, props):
        ...
        # Firefly control for the one-sample estimator weight value/pdf.
        #
        # `m_clamp` bounds the numerator: Model_M's output nonlinearity is
        # exp(), so a pre-activation the network never saw during training can
        # produce an arbitrarily large radiance.
        #
        # `pdf_eps` and `max_weight` bound the quotient, and the two do
        # different jobs. A sample whose pdf is at or below pdf_eps is
        # *rejected* (weight 0) rather than divided by a floor: flooring the
        # denominator silently returns a value that is neither the estimator
        # nor zero, whereas rejecting is a known, countable loss. max_weight
        # then caps whatever survives.
        #
        # The previous code did `value / dr.maximum(pdf, 1e-6)` with value
        # clamped at 100, which bounds nothing useful -- it permits a single
        # sample to carry a weight of 1e8 into the film. Both are biased;
        # bounded-and-biased beats unbounded-and-biased, and the bias here is
        # a pure energy loss in the tail, so it darkens rather than inventing
        # light. Raise max_weight if a render looks dim in the caustic-like
        # regions where the RDM pdf is a poor match for the learned lobe.
        self.m_clamp = props.get('m_clamp', 100.0)
        self.pdf_eps = props.get('pdf_eps', 1e-9)
        self.max_weight = props.get('max_weight', 1e4)
        ...
        # Stochastic detail from the RDM's second moment.
        #
        # A bin stores the mean of the paths that landed in it, and the gather
        # now also measures how far those paths scatter around that mean:
        # sqrt(var_m)/mean_cell_m exceeds 1 in 99.8% of well-populated cells,
        # with a median of 2.53. So the stored value is not a summary of a
        # tight distribution, it is the centre of one several times wider than
        # itself -- which is why the learned term renders as a flat fill.
        #
        # This restores the spread without touching the mean. With probability
        # p the shading point returns value/p and otherwise returns zero, so
        # E[gain] = 1 exactly and the render stays unbiased; only the variance
        # changes. Matching the measured coefficient of variation r needs
        # p = 1/(1 + r^2), which at the median r gives 13.5% of points lighting
        # up at 7.4x. Sparse bright points is what sparkle physically is.
        #
        # The draw is keyed on *surface position*, not on the sampler state.
        # An independent draw per sample would average straight back to the
        # mean at 32 spp and hand back the flat render; hashing position makes
        # the pattern stable under spp, as in stochastic glint models. Note
        # what that implies: the shading becomes position-dependent, so this
        # smuggles in a positional input -- using the measured variance to
        # decide how much per-position variation is justified, rather than
        # learning it.
        self.sparkle = bool(props.get('sparkle', False))
        # Edge length of the position quantisation cell, in scene units, which
        # sets the grain of the speckle. The stone's girdle radius is 1.
        self.sparkle_scale = props.get('sparkle_scale', 0.02)
        # Floor on p, and hence a ceiling of 1/p_min on the gain. The measured
        # spread reaches r = 15.6 at p99, which would ask for gains near 250
        # and manufacture fireflies out of thin cells.
        self.sparkle_p_min = props.get('sparkle_p_min', 0.02)
        self.rel_spread = None      # flat mi.Float over 4D cells, set by the loader
        self.spread_dims = None     # (theta_i, phi_i, theta_o, phi_o) bin counts
    ...
    def sample_residual(self, si, sample1, sample2, active):
        ...
        value = (dr.clamp(self.eval_model_m_wi(wi_f, wo, si.wavelengths),
                          0.0, self.m_clamp)
                 * self._sparkle_gain(si, wi_f, wo))
        pdf = self.pdf_m(wi_f, wo, active)
        weight = self._one_sample_weight(value, pdf, active)
        return wo, pdf, weight

    def _lobe_prob(self, si):
        """
        Probability of picking the analytic reflection lobe.

        This is the paper's Eq. 11 proportion. There, p_T = P(T | omega_i) is
        supplied by Model_T and the reflected share is what is left over, so
        with Model_T attached this returns 1 - P(T | omega_i) and the split
        follows Soh & Montazeri exactly.

        Without Model_T it falls back to Fresnel, which for a *smooth
        dielectric* is not a degradation but a closed form for the same
        quantity. Measured against this gather's own rdm_r / rdm_t:

            theta_i    R measured    Fresnel
             11.2 deg     0.1768      0.1721
             33.8 deg     0.1801      0.1745
             56.2 deg     0.2052      0.1998
             78.7 deg     0.3687      0.4019
             mean         0.2284      0.2371

        i.e. agreement to 3.8% with the correct angular trend. Model_T earns
        its place in the paper because a fibre bundle has no closed form for
        the transmitted fraction; a polished facet does. Keep the network path
        anyway, because the `explicit_entry=False` ablation is meant to
        reproduce their method rather than improve on it.

        Note this is unused under Route A (explicit_entry=True, the default):
        there the entry interaction is a real dielectric that computes the
        reflect/transmit split from Fresnel and Snell per ray, which subsumes
        both T and R exactly rather than approximating them.

        Clamped so neither lobe can become unsamplable.
        """
        if self.model_t is not None:
            p_reflect = 1.0 - self.eval_model_t(si.wi)
        else:
            p_reflect = self.fresnel_i(si)
        return dr.clip(p_reflect, 0.05, 0.95)
    ...
    def _sample_route_a(self, ctx, si, sample1, sample2, active):
        ...
        value_m = (dr.clamp(
            self.eval_model_m_wi(wi_f, wo_f, si.wavelengths), 0.0, self.m_clamp)
            * self._sparkle_gain(si, wi_f, wo_f))
        pdf_m = self.pdf_m(wi_f, wo_f, active)
        weight_m = self._one_sample_weight(value_m, pdf_m, active)
    ...
    def _sparkle_gain(self, si, wi, wo):
        """
        Position-hashed sparse gain with unit mean. See the sparkle block in
        __init__ for why this exists and why it is keyed on position.

        Returns 1.0 unmodified when the feature is off or the checkpoint
        carries no second moment, so every caller can apply it blindly.
        """
        if not self.sparkle or self.rel_spread is None or self.rdm_sampler is None:
            return mi.Float(1.0)

        s = self.rdm_sampler
        ti, pi_ = s._incoming_bin(*self._dir_to_spherical(wi))
        to, po = s._outgoing_bin(*self._dir_to_spherical(wo))
        flat = ((ti * s.phi_i_bins + pi_) * s.theta_o_bins + to) * s.phi_o_bins + po
        r = dr.gather(mi.Float, self.rel_spread, flat)

        p = dr.clip(1.0 / (1.0 + r * r), self.sparkle_p_min, 1.0)

        # Quantise the shading point onto a lattice and hash the cell index, so
        # every sample landing in the same cell draws the same variate. The
        # offset keeps coordinates positive before the unsigned cast; the stone
        # sits within a unit-ish radius of the origin, so 1024 is ample.
        q = si.p / self.sparkle_scale + 1024.0
        ix, iy, iz = (mi.UInt32(q.x), mi.UInt32(q.y), mi.UInt32(q.z))
        v0 = ix * 73856093 ^ iy * 19349663
        v1 = iz * 83492791
        u = mi.sample_tea_float32(v0, v1)

        return dr.select(u < p, 1.0 / p, 0.0)

    def _eval_m_clamped(self, si, wo):
        # Clamp the network first, then apply the gain. m_clamp exists to bound
        # Model_M's exp() output; clamping *after* the multiply would clip the
        # bright half of the sparse gain and quietly destroy its unit mean,
        # turning an unbiased reweighting into a systematic darkening. The
        # resulting spikes are bounded downstream by max_weight.
        return (dr.clamp(self.eval_model_m(si, wo), 0.0, self.m_clamp)
                * self._sparkle_gain(si, si.wi, wo))

    def _one_sample_weight(self, value, pdf, active):
        """
        Bounded one-sample estimator weight, value / pdf.

        Rejects samples at or below `pdf_eps` instead of flooring the
        denominator, then clamps the survivors at `max_weight`. See the
        firefly-control block in __init__ for why both guards are needed and
        what they cost. Every site that forms value/pdf must go through here,
        so the two lobes cannot drift onto different guards.
        """
        usable = active & (pdf > self.pdf_eps)
        weight = value / dr.select(usable, pdf, 1.0)
        weight = dr.minimum(weight, self.max_weight)
        return dr.select(usable, weight, mi.UnpolarizedSpectrum(0.0))
        ...
        if self.explicit_entry:
            ...
            value = dr.clamp(
                self.eval_model_m_wi(self._fold(si.wi, flip), wo,
                                     si.wavelengths), 0.0, self.m_clamp)
