...
class RDMAliasSampler:
    ...
    def __init__(self, rdm_t, rdm_m, solid_angles, count_i, min_count=1.0):
        ...
        # Empty rows still need a normalized proposal. A small uniform-sphere
        # mixture also covers positive neural predictions in unmeasured cells.
        uniform = np.broadcast_to(solid_angles_out.reshape(1, -1), flat_mass.shape)
        uniform = uniform / uniform.sum(axis=1, keepdims=True)
        pmf = np.divide(flat_mass, row_sum, out=uniform.copy(), where=row_sum > 0)
        pmf = 0.99 * pmf + 0.01 * uniform
    ...
    def sample_outgoing(self, theta_i, phi_i, u1, u2, u3):
        ...
        # dOmega = dphi d(cos(theta)): uniform theta would disagree with
        # the piecewise-constant solid-angle density returned below.
        theta_lo = mi.Float(to_idx) * (dr.pi / self.theta_o_bins)
        theta_hi = (mi.Float(to_idx) + 1.0) * (dr.pi / self.theta_o_bins)
        cos_theta = (1.0 - u2) * dr.cos(theta_lo) + u2 * dr.cos(theta_hi)
        theta_o = dr.acos(dr.clip(cos_theta, -1.0, 1.0))
...
def build_rdm_sampler(checkpoint_dir, filename="rdm.npz", min_count=1.0):
    ...
    if not os.path.exists(path):
        print(f"  ⚠ RDM data not found at {path} -- "
              f"direction sampling will fall back to uniform sphere")
        return None
