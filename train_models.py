...
from utils.rdm import solid_angle_grid
...
def parse_args():
    ...
    parser.add_argument("--from_samples", action='store_true',
                       help="Build Model_M's training set from checkpoints/<name>/samples.npz "
                            "by re-binning the raw gathered samples, instead of using the "
                            "histogram baked in at gather time. Lets --theta_o_bins / "
                            "--phi_o_bins choose the outgoing resolution here.")
    parser.add_argument("--theta_o_bins", type=int, default=8,
                       help="Outgoing theta resolution when --from_samples is set.")
    parser.add_argument("--phi_o_bins", type=int, default=16,
                       help="Outgoing phi resolution when --from_samples is set.")
    parser.add_argument("--width", type=int, default=None,
                       help="Hidden width of Model_M. Defaults to the render-safe 21.")
    parser.add_argument("--bands", type=int, default=None,
                       help="Fourier feature octaves for Model_M's input encoding, at "
                            "frequencies pi, 2pi, ... 2^(bands-1)*pi. Defaults to 0. "
                            "An MLP on raw coordinates is spectrally biased toward smooth "
                            "functions, so width alone buys nothing: measured on ks_k8, "
                            "29x the width at bands=0 recovered LESS RDM saturation "
                            "(63.0%) than the base model (64.0%), while width 128 with "
                            "bands 6 reached 96.5% and took per-bin hue correlation from "
                            "+0.085 to +0.900. The top band resolves features about 4-5 "
                            "degrees wide, which is the scale of a facet (6.5 deg) and of "
                            "a fire flash. Cost: encoded input width is 6*(1+2*bands), so "
                            "bands=6 makes the first layer 128x78 and the render needs "
                            "eval.py --tile to fit.")
...
def prepare_training_data_from_samples(sample_path, theta_i_bins, phi_i_bins,
                                       theta_o_bins, phi_o_bins):
    """
    Build Model_M's training set by binning raw gathered samples at a
    resolution chosen HERE rather than during the gather.

    The target is a density -- (fraction of paths from an incoming bin that
    exit into an outgoing bin) / solid_angle -- so it still has to be
    estimated by binning; a sample says "a path went here", not "the density
    here is X". What raw samples buy is that the bin size is no longer baked
    into the gather, so the outgoing resolution can be swept for the cost of a
    retrain instead of a fresh 20M-ray run.

    That is the knob that matters for appearance. At the 8x16 outgoing grid a
    cell spans 22.5 degrees while a facet subtends about 6.5 degrees from the
    eval camera, so every point on a facet reads one bin value and renders
    flat. At 32x64 a cell is 5.6 degrees, comparable to a facet.
    """
    d = np.load(sample_path)
    wi, wo, val = d['wi'], d['wo'], d['value']
    n = len(wi)

    theta_i = np.arccos(np.clip(wi[:, 2], -1, 1))
    phi_i = np.arctan2(wi[:, 1], wi[:, 0])
    theta_o = np.arccos(np.clip(wo[:, 2], -1, 1))
    phi_o = np.arctan2(wo[:, 1], wo[:, 0])

    ti = np.clip((theta_i / (np.pi / 2) * theta_i_bins).astype(int), 0, theta_i_bins - 1)
    pi_ = np.clip(((phi_i + np.pi) / (2 * np.pi) * phi_i_bins).astype(int), 0, phi_i_bins - 1)
    to = np.clip((theta_o / np.pi * theta_o_bins).astype(int), 0, theta_o_bins - 1)
    po = np.clip(((phi_o + np.pi) / (2 * np.pi) * phi_o_bins).astype(int), 0, phi_o_bins - 1)

    shape = (theta_i_bins, phi_i_bins, theta_o_bins, phi_o_bins)
    flat = np.ravel_multi_index((ti, pi_, to, po), shape)
    ncell = int(np.prod(shape))

    acc = np.zeros((ncell, 3))
    for c in range(3):
        acc[:, c] = np.bincount(flat, weights=val[:, c], minlength=ncell)
    # The denominator must be the number of ALL hit rays per incoming bin --
    # what compute_histogram_4d divides by -- not the number of multi-scatter
    # samples, which is smaller by hits/M (about 1.4x here). count_i is carried
    # in samples.npz for exactly this reason. Rescale too if only a fraction of
    # the multi-scatter paths were retained.
    if 'count_i' in d:
        count_in = np.asarray(d['count_i']).reshape(-1).astype(float)
        if 'n_m' in d and 'n_kept' in d:
            kept = float(np.sum(d['n_kept'])) / max(float(np.sum(d['n_m'])), 1.0)
            if kept > 0:
                acc /= kept
                print(f"    retained {100 * kept:.1f}% of multi-scatter paths; rescaled")
    else:
        print("    [warn] no count_i in samples.npz; normalising by sample count, "
              "which overestimates by hits/M")
        count_in = np.bincount(np.ravel_multi_index((ti, pi_), shape[:2]),
                               minlength=theta_i_bins * phi_i_bins).astype(float)

    # Same normalisation as compute_histogram_4d: per-incoming-bin mean, then
    # per unit outgoing solid angle.
    sa = solid_angle_grid(theta_o_bins, phi_o_bins)[0].reshape(-1)
    acc = acc.reshape(theta_i_bins * phi_i_bins, theta_o_bins * phi_o_bins, 3)
    with np.errstate(divide='ignore', invalid='ignore'):
        acc = acc / np.maximum(count_in, 1.0)[:, None, None]
    acc = acc / sa[None, :, None]
    acc = np.nan_to_num(acc, nan=0.0, posinf=0.0)

    observed = count_in > 0
    tic = (np.arange(theta_i_bins) + 0.5) * (np.pi / 2) / theta_i_bins
    pic = -np.pi + (np.arange(phi_i_bins) + 0.5) * (2 * np.pi) / phi_i_bins
    toc = (np.arange(theta_o_bins) + 0.5) * np.pi / theta_o_bins
    poc = -np.pi + (np.arange(phi_o_bins) + 0.5) * (2 * np.pi) / phi_o_bins

    X, Y = [], []
    for k in range(theta_i_bins * phi_i_bins):
        if not observed[k]:
            continue
        a, b = divmod(k, phi_i_bins)
        wiv = [np.sin(tic[a]) * np.cos(pic[b]), np.sin(tic[a]) * np.sin(pic[b]), np.cos(tic[a])]
        for j in range(theta_o_bins * phi_o_bins):
            c, e = divmod(j, phi_o_bins)
            X.append(wiv + [np.sin(toc[c]) * np.cos(poc[e]),
                            np.sin(toc[c]) * np.sin(poc[e]), np.cos(toc[c])])
            Y.append(acc[k, j])

    X = np.array(X, dtype=np.float32)
    Y = np.array(Y, dtype=np.float32)
    lit = int((Y.sum(axis=1) > 1e-6).sum())
    print(f"  Re-binned {n} raw samples at {theta_i_bins}x{phi_i_bins} incoming "
          f"x {theta_o_bins}x{phi_o_bins} outgoing")
    print(f"    outgoing cell   : {180.0 / theta_o_bins:.1f} x {360.0 / phi_o_bins:.1f} degrees")
    print(f"    training bins   : {len(X)}  ({lit} lit, {len(X) - lit} measured dark)")
    print(f"    samples per cell: {n / max(len(X), 1):.1f}")
    print(f"  Y range: [{Y.min():.4f}, {Y.max():.4f}]")
    return X, Y
...
def main():
    ...
    sample_path = os.path.join(checkpoint_dir, 'samples.npz')
    if args.from_samples:
        if not os.path.exists(sample_path):
            raise FileNotFoundError(
                f"--from_samples needs {sample_path}. Re-run gather_rdm.py with "
                f"--max_samples to produce it.")
        X_m, Y_m = prepare_training_data_from_samples(
            sample_path, rdm_m.shape[0], rdm_m.shape[1],
            args.theta_o_bins, args.phi_o_bins)
    else:
    ...
    if args.width is not None:
        m_kwargs['width'] = args.width
    if args.bands is not None:
        m_kwargs['bands'] = args.bands
    model_m = Model_M(**m_kwargs)
    print(f"  Model_M architecture: {model_m.arch()}")
